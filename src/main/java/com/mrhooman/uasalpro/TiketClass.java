...
import static com.mrhooman.uasalpro.Uasalpro.input;
...
public class TiketClass {
    ...
    public static void showTiket() {
        int index = 0;
        System.out.printf("%-2s | %-15s | %-10s | %-10s | %-4s | %-10s | %-10s |%n", "No", "Pemesan", "Kode Tiket",
                "Kode Event", "Jmlah", "Total", "Status");
        while (index < Uasalpro.listTiket.size()) {
            Uasalpro.Tiket currentTiket = Uasalpro.listTiket.get(index);
            System.out.printf("%-2s | %-15s | %-10s | %-10s | %-4s | %-10s | %-10s |%n", index + 1,
                    currentTiket.namaPemesan(), currentTiket.kode(), currentTiket.kodeEvent(), currentTiket.jumlah(),
                    currentTiket.totalHarga(), currentTiket.keterangan());
            index++;
        }
    }
    ...
    public static void showJenisTiket() {
        int index = 0;
        System.out.printf("%-2s | %-15s | %-10s | %-10s | %-10s |%n", "No", "Kode Event", "Jenis Tiket", "Harga",
                "Jumlah Maksimal");
        while (index < Uasalpro.listJenisTiket.size()) {
            Uasalpro.JenisTiket currentTiket = Uasalpro.listJenisTiket.get(index);
            System.out.printf("%-2s | %-15s | %-10s | %-10s | %-10s |%n", index + 1, currentTiket.kodeEvent(),
                    currentTiket.jenis(), currentTiket.harga(), currentTiket.maksimal());
            index++;
        }
    }
    ...
    public static void editJenisTiket(String kodeEvent) {
        ...
        while (index < Uasalpro.listJenisTiket.size()) {
            ...
            if (currentTiket.kodeEvent().equals(kodeEvent)) {
                System.out.print(
                        "Masukkan Nama Jenis Tiket ( Before : " + currentTiket.jenis() + " | ketik - untuk tetap ) : ");
                String namaJenis = input.nextLine();
                // Kondisi value akan diedit atau tidaknya
                if (namaJenis.equals("-")) {
                    namaJenis = currentTiket.jenis();
                }
                System.out.print(
                        "Masukkan Harga Tiket ( Before : " + currentTiket.harga() + " | ketik 0 untuk tetap ) : ");
                ...
                System.out.print("Masukkan maksimal tiket ( Before : " + currentTiket.maksimal()
                        + " | ketik 0 untuk tetap ) : ");
                ...
                // Inisiasi JenisTiket baru untuk mengganti JenisTiket yang dipilih dengan
                // property yang telah diinput
